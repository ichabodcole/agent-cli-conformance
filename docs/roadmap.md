...
The source is the Round 4 findings of `docs/reviews/2026-08-14-implementation-review.md`
(untracked, local to the maintainer's checkout), plus the scope decisions taken during the four
remediation phases that preceded this page. This is not a transcription of that review: it
adopts most of its ordering argument, departs from it in five places, and each departure is
named.
...
review's R3-1 named — where "implemented", "pass" and "planned" all become easier to overread
than the project intends — and the cheapest defence against it is that the spec and the
roadmap are different files.
...
1. [**Structured remediation**](#1-r4-4--remediation-becomes-structured-data) (R4-4) — the
   schema decision that gets more expensive with every consumer.
2. [**Version the contract**](#2-r4-6--version-the-contract-not-only-the-rules) (R4-6) — before
   anything downstream produces something a consumer can pin to.
3. [**Control the observation environment**](#3-r4-9--control-the-observation-environment-which-is-also-the-l0-safety-work)
   (R4-9, merged with the L0 safety capability deferred out of Phase 4).
4. [**Durable observation and replay**](#4-r4-1--durable-observation-and-replay) (R4-1) — the
   artifact, once there are coordinates to stamp on it and an environment worth describing.
5. [**Profiles and the outcome algebra**](#5-r4-2--r4-3--profiles-and-the-outcome-algebra)
...
The review closes Round 4 with a seven-step order. Five departures, each argued in the section
it belongs to and collected here so none of them is silent:
...
- **Versioning is split from the artifact and put strictly before it.** The review bundles both
  ...
  to exist first.
- **R4-9 is merged with the deferred L0 safety work and promoted ahead of the artifact.** The
  review keeps them apart — safety under R2-1, environment under R4-9 at P2. They are one
  mechanism seen from two sides, and building them separately means building the same
  environment-construction path twice.
- **R4-3 is merged into R4-2 rather than sitting a step ahead of it.** The review resolves the
  outcome model at step 2 and adds profiles at step 4. A streaming profile _is_ its completion
  semantics; separated, one half has no subject and the other has a placeholder at its centre.
...
[already discharged](#already-discharged).

---

## 1. R4-4 — remediation becomes structured data
...
**Why first, against the review's own ordering.** The review rates this P1 and then does not
place it in its seven steps at all. Ranked by consequence it is first, for three reasons that
compound: `acc` emits `next` in every success envelope right now, so consumers can start
depending on the string shape today; the change is small, touching the envelope and one
declaration; and the cost curve is the one the project already argued about somewhere else. The
...
schema is the same shape of decision, at the same stage, with a safety consequence attached
rather than an ergonomic one.

**Blocked on.** Nothing. The design argument is written; what is missing is the schema and the
emitter.

## 2. R4-6 — version the contract, not only the rules
...
**Why split from the artifact.** The review bundles versioning and the artifact into one step.
The artifact's own required-field list opens with "spec, checker, report, and artifact format
versions" — so the coordinate set has to exist before the artifact schema can embed it. Doing
them together means designing the fields twice, once provisionally.
...
## 3. R4-9 — control the observation environment, which is also the L0 safety work
...
**And, in the same breath, the capability deferred out of Phase 4.** The review named five
measures as the fix for the L0 safety overclaim. Phase 4 corrected the **claim** — the README
and the safety prose now say that `L0` is risk-reduced rather than inert, that `acc check`
executes third-party code, and that a fresh temporary working directory redirects relative paths
and nothing else — and **did not build the capability**. Stated plainly, because a corrected
claim reads like progress and is not:
...
**One correction to the deferred item as worded.** "A dry-run mode printing the complete planned
argv list **before execution**" is not achievable as stated. A checker's probes are a function of
discovery — `probes: (d: Discovery) => Invocation[]` — and discovery is produced by running the
target's help path. So the kit can print the complete plan _after one `--help` invocation_, or
the discovery-independent part of it before any execution at all, but not both. The two are
genuinely different products (an audit of what will run next, versus a preflight for a binary
you have not yet consented to run once), and whichever is built should say which it is. This is
otherwise the cheapest item on this page, and it is a safety mitigation rather than the adoption
nicety the review files it as.

## 4. R4-1 — durable observation and replay
...
## 5. R4-2 + R4-3 — profiles and the outcome algebra
...
Together with the outcome algebra those profiles need: streams get a machine-readable terminal
event, a non-zero process outcome, and an unambiguous way for a consumer to know the preceding
stream is incomplete; batches get per-item outcomes plus a defined overall result.
...
the algebra has no declared subject to attach to; written second, the stream profile ships with a
placeholder where its central rule goes. One step.
...
## 6. R4-7 — the portable declaration IR
...
## 8. R4-8 — test the checker as a measurement instrument
...
every normative clause to either evidence or `unverified`. The review puts it well: this is the
difference between having tests for a checker and knowing what the checker can validly claim.

**This phase produced an instance of the gap.** The dogfood suite carried its own copy of F1's
credential patterns, looser than the shipped ones — no word boundary, no length floor — so `sk-`
matched inside ordinary English. It fired on the word "risk-reduced" in README prose during the
previous phase, and the prose was reworded to dodge it. Nothing was wrong with the shipped
checker; the instrument standing beside it had never been tested in either direction, against
prose it must ignore or against a credential it must catch, and so did neither correctly. It has
those tests now. One pattern, in one file, found by accident — which is the argument for the
...
The ordering defers the sweep. It does not defer the discipline, and the F1 canary is the
standing example of what happens when it does.

## 9. Adoption surfaces

`acc init`, CI integration beyond this repository's own gate, SARIF or JUnit report exports, and
the adoption guide the review calls the largest remaining navigation gap.

Last, agreeing with the review, and for a reason worth making explicit: an export format is a
consumer, and it pins to the report shape. Shipping SARIF before step 2 creates precisely the
accidental compatibility promise that step exists to prevent.

The one item lifted out of this group is the probe-plan dry run, which belongs with step 3 — it
is a safety mitigation, not an ergonomic one.

---

## Design guidance that is not yet normative

Review finding R3-1 identified five requirements that live only in concept-page prose. Under this
project's own contract, only rule pages are normative, so none of them binds anything:
...
hardening recommendations. Two Round 3 findings remain and are placed above rather than here:
R3-1's normative-scope question is settled at
[design guidance](#design-guidance-that-is-not-yet-normative), and R3-8's adoption guide sits at
