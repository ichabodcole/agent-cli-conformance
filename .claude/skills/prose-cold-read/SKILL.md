...
`references/CATALOGUE.md`, beside this file, holds the defect definitions with worked examples and boundary
cases. It is maintained by the `/prose-defect` command.

**The reader must be a fresh subagent, and it must not be briefed.** Four of the five catalogued
defects are invisible to whoever wrote the text, for a single reason: the writer holds something
the reader does not, and the sentence does not supply it. You cannot notice the absence of
something you are still carrying. Reading your own draft against this catalogue does not work,
and neither does reading someone else's after they explain it to you.

## Steps

1. **Sweep first.** Run the greps in the catalogue's entries over the target and fix what they
   plainly catch, so the read has less to find. A clean sweep is not a clean document — entry 3
   records the same pattern under-reporting three times running.
...
Passages in the catalogue have carried two and three.
