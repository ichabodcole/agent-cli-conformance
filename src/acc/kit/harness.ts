...
# writes a recorded-surface batch to ${input.out}, or wherever \`-o PATH\` says.
#
#   sh <this script>                   -> ${input.out}
#   sh <this script> -o reports/b.json -> reports/b.json   (the directory must already exist)
#
...
#
# THAT WARNING IS ABOUT THIS SCRIPT, NOT ABOUT THE BATCH IT WRITES. Moving, copying or renaming the
# output afterwards changes nothing it derives — an adopter asked, and the warning did not say. Use
# \`-o\` and the question does not arise.
#
...
# WHERE THE BATCH GOES. \`-o PATH\` overrides the default above, at RUN time.
#
# \`acc probe-plan\` has \`--out\` for this script; this script had none for its own output, so the
# batch always landed in the working directory. An adopter also asked whether MOVING the file
# afterwards counts as editing a script that says do not edit it. It does not, and with this flag
# the question stops arising: ask for the destination and the file arrives there.
#
# PARSED HERE, BEFORE THE DIRT CHECK BELOW, and the order is load-bearing rather than tidy: that
# check excludes "$OUT" from \`git status\` so the batch cannot report the tree it is measuring as
# dirty. Setting OUT after the exclusion is built would append \`-dirty\` to BUILD on every run of
# the documented workflow from a clean checkout.
while [ $# -gt 0 ]; do
  case "$1" in
    -o|--out)
      [ $# -ge 2 ] || { printf 'acc harness: %s needs a path\\n' "$1" >&2; exit 2; }
      OUT=$2; shift 2 ;;
    -h|--help)
      printf 'usage: sh %s [-o PATH]\\n\\nWrites a recorded-surface batch to PATH (default %s).\\n' "$0" "$OUT"
      exit 0 ;;
    --) shift; break ;;
    *)
      printf 'acc harness: unknown argument %s (only -o PATH is accepted)\\n' "$1" >&2
      exit 2 ;;
  esac
done
# FAIL NOW RATHER THAN AFTER EVERY PROBE. The batch is written at the very end, so a mistyped
# destination would otherwise be discovered only once the whole capture had run and be paid for by
# repeating it. Creating the directory is deliberately NOT the behaviour: a typo that silently
# produces a new tree is worse than one that stops.
_outdir=$(dirname "$OUT")
[ -d "$_outdir" ] || { printf 'acc harness: no such directory: %s\\n' "$_outdir" >&2; exit 2; }
...
# \`set --\` deliberately consumes $@, which the option loop above has already emptied.
set --
if [ -n "$_top" ]; then
